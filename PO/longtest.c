...
int main(int argc, char *argv[]) {
    ...
    for (i = optind; i < argc; i++) {
	    ...
	    const Bag *b = create(dohash, doNothing, ncmp, 0L, 0.0, nhash);
            if (b != NULL) {
                printf("success\n");
                b->destroy(b);
            } else
	    ...
	    const Bag *b = create(dohash, doNothing, ncmp, 0L, 0.0, nhash);
            long values[] = {1, 1, 2, 3, 3, 3, 4, 5, 5, 5, 5};
            int i, success = 1;
            for (i = 0; i < 11; i++) {
                if (! b->add(b, (void *)values[i])) {
                    success = 0;
	    ...
	    const Bag *b = create(dohash, doNothing, ncmp, 0L, 0.0, nhash);
            ...
            long *array;
            for (i = 0; i < 11; i++)
                (void)b->add(b, (void *)values[i]);
            array = (long *)b->toArray(b, &len);
            if (array != NULL && len == 11L) {
                for (i = 0; i < 11; i++) {
                    if (findLong(array[i], values, 11L))
                        success++;
                }
             }
             if (success)
