...
int main(UNUSED int argc, UNUSED char *argv[]) {
    ...
    while (fgets(buf, BUFSIZ, stdin) != NULL) {
        sprintf(request, "%s|%u|%s|%s", ipaddr, svcPort, SERVICE, buf); // packed datat together then sends
...
//for each server service then you need a thread for each sercive and then join.